...
def main():
    ...
    if output:
        try:
            output_func = eval('lambda entry: ' + output)
        except:
            pywikibot.exception()
            pywikibot.error('Cannot compile output command: {0}'.format(output))
            exit(1)
    else:
        output_func = lambda entry: pywikibot.output(entry)

    if command:
        try:
            filter_func = eval('lambda entry: ' + command)
        except:
            pywikibot.exception()
